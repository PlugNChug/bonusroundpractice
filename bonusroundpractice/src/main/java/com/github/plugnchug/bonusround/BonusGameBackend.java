...
public class BonusGameBackend {
    ...
    public void calculateWordPosition(List<String> words, int answerLen) {
        ...
        boolean longestOnTopOrBottom = false;

        if (Game.customPuzzle) {
            wordLengths.add(CustomPuzzleWindow.topRow.length());
            wordLengths.add(CustomPuzzleWindow.topMiddleRow.length());
            wordLengths.add(CustomPuzzleWindow.bottomMiddleRow.length());
            wordLengths.add(CustomPuzzleWindow.bottomRow.length());

            for (int i = 0; i < wordLengths.size(); i++) {
                if (wordLengths.get(i) > largestWordLength) {
                    largestWordLength = wordLengths.get(i);
                    if (i == 0 || i == 3) {
                        startPos = (MIDDLE_FIRST_FOURTH - (wordLengths.get(i) / 2));
                        longestOnTopOrBottom = true;
                    } else {
                        startPos = (MIDDLE_SECOND_THIRD - (wordLengths.get(i) / 2));
                        longestOnTopOrBottom = false;
                    }
                    
                }
            }

            int middleRowsStartPos = startPos;
            if (longestOnTopOrBottom) {
                middleRowsStartPos++;
            } else {
                startPos--;
            }
            animation.animateReveal(CustomPuzzleWindow.topRow, FIRST_ROW, startPos);
            animation.animateReveal(CustomPuzzleWindow.topMiddleRow, SECOND_ROW, middleRowsStartPos);
            animation.animateReveal(CustomPuzzleWindow.bottomMiddleRow, THIRD_ROW, middleRowsStartPos);
            animation.animateReveal(CustomPuzzleWindow.bottomRow, FOURTH_ROW, startPos);
        } else {
